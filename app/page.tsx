    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="border-b border-gray-200 bg-white sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Tabs value={activeMenu} onValueChange={(val) => setActiveMenu(val as 'manual' | 'database' | 'file')} className="w-full">
            <TabsList className="grid w-full grid-cols-3 rounded-none bg-transparent border-b-0 h-auto p-0">
              <TabsTrigger 
                value="manual" 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* MENU 1: Búsqueda Manual - Tu código original intacto */}
        {activeMenu === 'manual' && (
        ...
        {/* MENU 2: Leer Base de Datos - Nuevo componente */}
        {activeMenu === 'database' && (
          <TabDatabaseRead 
            onPrint={(skus) => {
              console.log('Imprimiendo desde BD:', skus);
              alert(`Enviando ${skus.length} etiqueta(s) a imprimir...`);
            }} 
          />
        )}

        {/* MENU 3: Cargar Archivo TXT - Nuevo componente */}
        {activeMenu === 'file' && (
          <TabFileUpload 
            onPrint={(skus) => {
