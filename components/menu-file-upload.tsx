...
export function MenuFileUpload() {
  ...
  // Buscar SKUs en productos
  const validSkus = uploadedSkus.filter(sku => 
    mockProducts.some(p => p.codigo === sku)
  );
  ...
  // Productos encontrados con oferta
  const productsWithOffer = mockProducts.filter(p => 
    validSkus.includes(p.codigo) && p.oferta
  );

  // Productos encontrados sin oferta
  const productsWithoutOffer = mockProducts.filter(p => 
    validSkus.includes(p.codigo) && !p.oferta
  );
            ...
            {!fileName ? (
              <div className="border-2 border-dashed border-purple-300 rounded-lg p-6 text-center hover:border-purple-400 transition">
                <label className="cursor-pointer block">
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-gray-700">Click para cargar un archivo</p>
                    <p className="text-xs text-gray-500">o arrastra tu archivo aquí</p>
                  </div>
                </label>
              </div>
            ) : (
              <div className="bg-white p-4 rounded-lg border border-green-200 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-600">{uploadedSkus.length} SKUs cargados</p>
                </div>
                <Button
                  size="sm"
                  variant="outline"
            ...
            <Card className="border-red-200 bg-red-50">
              ...
              <CardContent>
                <div className="space-y-2">
                  {invalidSkus.map(sku => (
                    <div key={sku} className="bg-white p-2 rounded text-sm font-mono flex items-center justify-between border border-red-100">
      ...
      {uploadedSkus.length === 0 && !fileName && (
        <Card className="border-dashed border-gray-300">
          <CardContent className="py-12 text-center">
            <p className="text-gray-500">Carga un archivo TXT para comenzar</p>
          </CardContent>
        </Card>
      )}
    </div>
